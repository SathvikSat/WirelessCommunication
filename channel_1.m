...
h_unpad_11 = randn(L, 1) + 1i * randn(L, 1);
%h_unpad_1 = 1;
...
h_unpad_12 = randn(L, 1) + 1i * randn(L, 1);
h_unpad_21 = randn(L, 1) + 1i * randn(L, 1);
h_unpad_22 = randn(L, 1) + 1i * randn(L, 1);
%h_unpad_2 = 1;
...
channel_faded_data_y1 =  conv(h_unpad_11, tx_cp) + cconv(h_unpad_12, tx_cp2);
channel_faded_data_y2 =  conv(h_unpad_21, tx_cp) + cconv(h_unpad_22, tx_cp2);
%H_mat = [1, 1; 1, 1];
%Tx_CP = [tx_cp'; tx_cp2'];
%test = H_mat * Tx_CP;
